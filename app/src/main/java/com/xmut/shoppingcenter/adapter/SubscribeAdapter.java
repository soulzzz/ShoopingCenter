...
public class SubscribeAdapter extends RecyclerView.Adapter<SubscribeAdapter.Myholder>{
    ...
    public void onBindViewHolder(@NonNull Myholder holder, int position) {
        ...
        Glide.with(context).load(StringUtil.getFirstSplitBySymbol(subscribes.get(position).getStore_img(),"|") ).placeholder(R.drawable.ic_error).into(holder.store_icon);
        ...
        holder.store_icon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                UserManage.getInstance().Jump((Activity) context, CheckStoreActivity.class,subscribes.get(position).getStore_id(),"store_id");
            }
