...
public class ProductGridLayoutAdapter extends RecyclerView.Adapter<ProductGridLayoutAdapter.Myholder>{
    Context context;
    List<Product> productList;
    String productimg;

    public ProductGridLayoutAdapter(Context context) {
        this.context = context;
    }
    public ProductGridLayoutAdapter(Context context, List<Product> productList) {
        this.context = context;
    ...
    public void setProductList(List<Product> productList) {
        for(int i=0;i<productList.size();i++){
            System.out.println("ID为"+productList.get(i).getProduct_id());
        }
        this.productList =productList;
    ...
    public Myholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = View.inflate(parent.getContext(),R.layout.item_product_grid,null);
