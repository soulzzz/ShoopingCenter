...
public class ChooseCouponAdatper extends RecyclerView.Adapter<ChooseCouponAdatper.Myholder> {
    Context context;
    List<Coupon> couponList;
    List<String> time = TimeUtil.getInstance().getFormatTime();
    double price;

    public ChooseCouponAdatper(Context context, List<Coupon> couponList, double price) {
    ...
    public void onBindViewHolder(@NonNull Myholder holder, int position) {
        ...
        holder.value.setText(OrderUtil.PriceFix(couponList.get(position).getValue() ));
        if(couponList.get(position).getStarttime().compareTo(time.get(1) ) <=0 && couponList.get(position).getEndtime().compareTo(time.get(1) ) >=0 && price > couponList.get(position).getRequirement() ){
            ...
            if( couponList.get(position).getEndtime().compareTo(time.get(1) ) <0 ){
